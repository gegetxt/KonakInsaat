...
$(function() {
    ...
    wind.on("scroll", function () {
        ...
        if (bodyScroll > 100) {
            ...
            // Logo yolunu data attribute'undan al, yoksa varsayılan kullan
            var scrollSrc = logo.data('scroll-src') || logo.data('default-src') || '/img/logo.png';
            logo.attr('src', scrollSrc);
        } else {
            navbar.removeClass("nav-scroll");
            // Logo yolunu data attribute'undan al, yoksa varsayılan kullan
            var defaultSrc = logo.data('default-src') || '/img/logo.png';
...
// Preloader - Pace + emniyet fallback
paceOptions = {
    ajax: true,
    document: true,
    eventLag: false
...
function hidePreloaderSafe() {
    ...
    if ($p.length && $p.is(':visible')) {
        $p.stop(true, true).fadeOut(600);
    }
}
Pace.on('done', function() {
    setTimeout(hidePreloaderSafe, 300);
});
$(window).on('load', function(){
    setTimeout(hidePreloaderSafe, 600);
});
// Son çare: 5 sn sonra yine gizlemeyi dene
setTimeout(hidePreloaderSafe, 5000);
